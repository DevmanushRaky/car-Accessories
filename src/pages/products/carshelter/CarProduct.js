...
function CarProduct() {
  const { carshelterId } = useParams();

  const shelter = CarSheltersProduct.find((shelter) => shelter.carshelterId === carshelterId);

  if (!shelter) {
    return (
      <Helmet title="Car Shelter Product">
  ...
  const handleShare = async () => {
    try {
      ...
      if (navigator.share) {
        ...
        // const productUrl = `http://localhost:3000/carproduct/${carshelterId}`; // Replace  with your website domain
        const productUrl = `https://vjhc.netlify.app/carproduct/${carshelterId}`;
